...
export default async function CharacterCodexPage({
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6 p-6 md:p-8">
      ...
      >
        <ChevronLeft className="h-3 w-3" /> {ch.name}
      </Link>
      <header>
        ...
        <h1 className="font-serif text-2xl font-semibold tracking-tight">
          What we know — {ch.name}
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Facts extracted from prose (confirmed and tentative). Read-only.
        </p>
      </header>
      ...
      <div className="space-y-6">
        {byPred.size === 0 ? (
          <p className="text-sm text-muted-foreground">
            No continuity claims linked to this character yet.
          </p>
        ) : (
